define(function(require,exports) {
	...
	function Container(config,parent,idObject){
		...
		if (util.isString(this.config.target)){
	...
	Extend(
		...
		,{
			 ...
			 * @param  {Boolean} el      是否直接导出单元jq对象
			 * @return {Mix}             检索出来的单元
			 */
			,get:function(index,el){
				index = +index;
				if(!isNaN(index)){
					index = _getElByIndex(this.gridCells,index);
					return el && index && index.el || index;
				}
			}
			/**
			 * 获取指定行
